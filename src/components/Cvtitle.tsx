...
export default function CvTitle({ name, cards } : Props) {
    return (
        <div>
            ...
            {cards.map(({title, icon = "", company, type = "", date_start, date_end, description=undefined}) => (
                <CvCard title={title} icon={icon} company={company} date_start={date_start} date_end={date_end} type={type} description={description} />
            ))}
        </div>
    )
}